...
const replaceLinkTag = (stylePath, position, tags, replacementTag) => {
  ...
  if (position === 'head-top') {
    tags.unshift(replacementTag);
  } else if (position === 'head-bottom') {
...
const replaceLinkTagWithStyleTag = (cssFilename, position, pluginArgs, compilation, minifier) => {
  ...
  replaceLinkTag(stylePath, position, pluginArgs.head, replacementTag);
};
